...
BEGIN
    ...
    FROM auth.get_resource_access_flags(__user_id_2, 'test-corr-flags-1b', 'document', '{"id": 2001}'::jsonb)
    INTO __flags;
...
BEGIN
    ...
    PERFORM auth.deny_resource_access('test', __user_id_1, 'test-corr-flags-2b', 'document', '{"id": 2002}'::jsonb,
        __user_id_2, _access_flags := array['write']);

    -- Get effective flags — should exclude 'write'
    SELECT array_agg(__access_flag order by __access_flag)
    FROM auth.get_resource_access_flags(__user_id_2, 'test-corr-flags-2c', 'document', '{"id": 2002}'::jsonb)
    INTO __flags;
...
BEGIN
    ...
    -- Check sources
    SELECT count(*) FROM auth.get_resource_access_flags(__user_id_2, 'test-corr-flags-3c', 'document', '{"id": 2003}'::jsonb)
    WHERE __source = 'direct' INTO __direct_count;

    SELECT count(*) FROM auth.get_resource_access_flags(__user_id_2, 'test-corr-flags-3d', 'document', '{"id": 2003}'::jsonb)
...
BEGIN
    ...
    -- Document {"id": 2002} has: group grants (read, write, delete) + user deny (write)
    SELECT count(*) FROM auth.get_resource_grants(__user_id_1, 'test-corr-grants-4a', 'document', '{"id": 2002}'::jsonb)
    INTO __total_count;

    SELECT count(*) FROM auth.get_resource_grants(__user_id_1, 'test-corr-grants-4b', 'document', '{"id": 2002}'::jsonb)
...
BEGIN
    ...
    SELECT exists(
        SELECT 1 FROM auth.get_resource_grants(__user_id_1, 'test-corr-grants-5a', 'document', '{"id": 2002}'::jsonb)
        WHERE __user_display_name IS NOT NULL
    ) INTO __has_user_name;

    SELECT exists(
        SELECT 1 FROM auth.get_resource_grants(__user_id_1, 'test-corr-grants-5b', 'document', '{"id": 2002}'::jsonb)
