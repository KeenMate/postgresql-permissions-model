...
from start_version_update('1.15',
													'Fix create_tenant and update_tenant',
													_component := 'keen_auth_permissions');

create or replace function unsecure.duplicate_perm_set(
	_created_by text, _user_id bigint, _source_perm_set_code text, _source_tenant_id int, _new_title text,
	_new_tenant_id int)
...
begin
	...
	if __source_perm_set is null then
		perform error.raise_52282(_source_perm_set_code);
	end if;

	insert into auth.perm_set(created, created_by, modified, modified_by, tenant_id, title, is_system,
														is_assignable, code)
	values ( now(), _created_by, now(), _created_by, _new_tenant_id, _new_title
				 , __source_perm_set.is_system, __source_perm_set.is_assignable, helpers.get_code(_new_title))
	returning * into __created_perm_set;

	-- copy assigned permissions

	insert into auth.perm_set_perm (created, created_by, perm_set_id, permission_id)
	select now(), _created_by, __created_perm_set.perm_set_id, permission_id
...
begin
	...
	perform unsecure.duplicate_perm_set(_created_by, _user_id, 'tenant_admin', 1,
																			'Tenant admin',
	...
	perform unsecure.duplicate_perm_set(_created_by, _user_id, 'tenant_member', 1,
																			'Tenant member',
																			__last_item.tenant_id::int);
	perform unsecure.assign_permission(_created_by, _user_id
