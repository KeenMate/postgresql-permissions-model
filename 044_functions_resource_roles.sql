...
-- ============================================================================
-- 1. auth.create_resource_role
-- ============================================================================
-- Register a resource role with its flags. The role is global (not tenant-scoped).
-- _tenant_id is used only for the RBAC permission check.
--
-- Example:
--   select * from auth.create_resource_role('app', 1, 'setup',
--       'folder_editor', 'folder', 'Editor',
--       _access_flags := array['read', 'write', 'delete', 'export'],
--       _description := 'Full editing access to a folder');
--
...
begin
    ...
    -- Insert role
    insert into const.resource_role (code, resource_type, title, description, source)
    values (_code, _resource_type, _title, _description, _source)
    ...
    -- Return the created/existing role with aggregated flags
    return query
        select r.code, r.resource_type, r.title, r.description, r.is_active, r.source,
...
-- ============================================================================
-- 2. auth.ensure_resource_roles
-- ============================================================================
-- Bulk-ensure roles from a JSONB array. Idempotent.
--
-- Each element:
--   code          (required) — role code
--   resource_type (required) — which resource_type the role is for
--   title         (required) — display title
--   description   (optional)
--   access_flags  (optional) — array of valid flag codes
--   source        (optional) — overrides the _source parameter per item
--
-- _is_final_state: when true, roles sharing the same _source that are NOT
-- in the input list are deactivated (is_active = false). Roles are never
-- deleted automatically — deactivation preserves audit history.
--
-- Example:
--   select * from auth.ensure_resource_roles('app', 1, 'setup', '[
--       {"code": "folder_reader", "resource_type": "folder", "title": "Reader",
--        "access_flags": ["read"]},
--       {"code": "folder_editor", "resource_type": "folder", "title": "Editor",
...
begin
    ...
    loop
        ...
        -- Validate flags globally and per-type
        if _access_flags is not null then
            perform unsecure.validate_access_flags(_access_flags);
        ...
        if exists (select 1 from const.resource_role where code = _code) then
            ...
            set title       = _title,
                ...
                source      = coalesce(_item_source, source),
                is_active   = true
            where code = _code;
        else
            insert into const.resource_role (code, resource_type, title, description, source)
            values (_code, _res_type, _title, _desc, _item_source);
        end if;

        -- Sync flags (delete-then-insert for exact set)
        if _access_flags is not null then
            delete from const.resource_role_flag
            where resource_role_code = _code
    ...
    -- Final state: deactivate roles from this source not in the input list
    if _is_final_state and _source is not null then
        for _existing_code in
            select r.code
            from const.resource_role r
            where r.source = _source
              and r.is_active = true
              and r.code not in (select value->>'code' from jsonb_array_elements(_roles))
        loop
            update const.resource_role
            set is_active = false
            where code = _existing_code;
        end loop;
    end if;

    -- Return all processed roles
    return query
        select r.code, r.resource_type, r.title, r.description, r.is_active, r.source,
               (select array_agg(rrf.access_flag_code order by rrf.access_flag_code)
                from const.resource_role_flag rrf where rrf.resource_role_code = r.code)
...
-- ============================================================================
-- 3. auth.update_resource_role
-- ============================================================================
-- Update mutable properties of a resource role.
-- code and resource_type are immutable.
--
...
-- ============================================================================
-- 11. auth.get_resource_role_assignments
-- ============================================================================
-- List all role assignments on a specific resource.
--
...
begin
    ...
    left join const.resource_role rr on rr.code = rra.role_code
