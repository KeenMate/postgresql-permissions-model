create table public.__version
(
    version_id         integer generated always as identity
        primary key,
    component          text                     default 'main'::text not null,
    version            text                                          not null,
    title              text                                          not null,
    description        text,
    execution_started  timestamp with time zone default now()        not null,
    execution_finished timestamp with time zone
);

create unique index uq_version
    on public.__version (component, version);

create table public._template_created
(
    created    timestamp with time zone default now()           not null,
    created_by text                     default 'unknown'::text not null
        constraint _template_created_created_by_check
...
(
    ...
    modified_by text                     default 'unknown'::text not null
        constraint _template_timestamps_modified_by_check
            check (length(modified_by) <= 250),
    constraint _template_created_created_by_check
        check (length(created_by) <= 250)
)
    inherits (public._template_created);

create table const.sys_param
(
    sys_param_id integer generated always as identity
        primary key,
...
create index ix_user_event_data
    on auth.user_event using gin (event_data pg_catalog.jsonb_path_ops);
...
create index ix_token_token_data
    on auth.token using gin (token_data pg_catalog.jsonb_path_ops);
...
(
    ...
    data_payload     jsonb,
    nrm_search_data  text generated always as (helpers.normalize_text(message)) stored not null,
    constraint _template_created_created_by_check
        check (length(created_by) <= 250)
)
    inherits (public._template_created);

create index ix_journal
    on public.journal (tenant_id, data_group, coalesce(data_object_id, 0::bigint), coalesce(data_object_code, '-'::text));

create index ix_journal_message
...
(
    external_group_member_id bigint generated always as identity
        primary key,
    user_group_id            integer not null
        references auth.user_group
            on delete cascade,
    user_group_mapping_id    integer not null
        references auth.user_group_mapping
            on delete cascade,
...
create or replace function helpers.is_empty_string(_text text) returns boolean
    immutable
    parallel safe
    cost 1
    language sql
...
create or replace function helpers.is_not_empty_string(_text text) returns boolean
    immutable
    parallel safe
    cost 1
    language sql
...
create or replace function helpers.compare_jsonb_objects(_first jsonb, _second jsonb) returns jsonb
    immutable
    parallel safe
    cost 1
    language sql
...
create or replace function helpers.compute_jsonb_hash(_data jsonb, _normalize_text boolean DEFAULT false, _field_value_separator text DEFAULT ':'::text, _fields_separator text DEFAULT '|'::text) returns text
    immutable
    strict
    cost 0
    language sql
...
create or replace function helpers.normalize_jsonb_values(_data jsonb) returns jsonb
    immutable
    strict
    cost 0
    language sql
...
create or replace function helpers.delete_jsonb_fields(_data jsonb, _fields_to_delete text[] DEFAULT NULL::text[]) returns jsonb
    immutable
    strict
    parallel safe
    cost 0
...
create or replace function error.raise_52101(_username text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52102(_normalized_email text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52103(_user_id bigint, _email text DEFAULT NULL::text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52104(_user_id bigint) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52105(_user_id bigint) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52106(_email text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52107(_provider_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52108(_tenant_id text, _username text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52109(_user_id bigint, _perm_codes text[], _tenant_id integer DEFAULT 1) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52110(_user_id bigint, _provider_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52111(_user_id bigint, _provider_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52112(_user_id bigint) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52171(_user_group_id integer) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52172(_user_group_id integer) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52173(_user_group_id integer) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52174() returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52175(_perm_set_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52176(_perm_set_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52177(_perm_set_id integer, _tenant_id integer DEFAULT 1) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52178() returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52179(_parent_full_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52271(_user_group_id integer) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52272() returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52273() returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52274() returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52275(_permission_full_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52276() returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52277() returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52278(_token_uid text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52279(_token_uid text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52180(_permission_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52181(_permission_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52282(_perm_set_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52283(_perm_set_code text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52301(_api_key text) returns void
    language plpgsql
as
$$
begin
...
create or replace function error.raise_52401(_user_id bigint, _user_group_id integer, _tenant_id integer DEFAULT 1) returns void
    language plpgsql
as
$$
begin
...
create or replace function public.add_journal_msg_jsonb(_created_by text, _user_id bigint, _msg text, _data_group text DEFAULT 'system'::text, _data_object_id bigint DEFAULT NULL::bigint, _payload jsonb DEFAULT NULL::jsonb, _event_id integer DEFAULT NULL::integer, _data_object_code text DEFAULT NULL::text, _tenant_id integer DEFAULT 1) returns SETOF journal
    rows 1
    language plpgsql
as
$$
...
create or replace function public.add_journal_msg(_created_by text, _user_id bigint, _msg text, _data_group text DEFAULT 'system'::text, _data_object_id bigint DEFAULT NULL::bigint, _payload text[] DEFAULT NULL::text[], _event_id integer DEFAULT NULL::integer, _data_object_code text DEFAULT NULL::text, _tenant_id integer DEFAULT 1) returns SETOF journal
    language sql
as
$$
...
create or replace function auth.update_provider(_modified_by text, _user_id bigint, _provider_id integer, _provider_code text, _provider_name text, _is_active boolean DEFAULT true)
    returns TABLE(__provider_id integer)
    rows 1
    language plpgsql
as
...
create or replace function auth.enable_provider(_modified_by text, _user_id bigint, _provider_code text)
    returns TABLE(__provider_id integer)
    rows 1
    language plpgsql
as
...
create or replace function auth.disable_provider(_modified_by text, _user_id bigint, _provider_code text)
    returns TABLE(__provider_id integer)
    rows 1
    language plpgsql
as
...
create or replace function auth.update_user_group(_modified_by text, _user_id bigint, _user_group_id integer, _title text, _is_assignable boolean, _is_active boolean, _is_external boolean, _is_default boolean, _tenant_id integer DEFAULT 1)
    returns TABLE(__user_group_id integer)
    rows 1
    language plpgsql
as
...
create or replace function auth.enable_user_group(_modified_by text, _user_id bigint, _user_group_id integer, _tenant_id integer DEFAULT 1)
    returns TABLE(__user_group_id integer, __is_active boolean, __is_assignable boolean, __modified timestamp with time zone, __modified_by text)
    rows 1
    language plpgsql
as
$$
...
create or replace function auth.disable_user_group(_modified_by text, _user_id bigint, _user_group_id integer, _tenant_id integer DEFAULT 1)
    returns TABLE(__user_group_id integer, __is_active boolean, __is_assignable boolean, __modified timestamp with time zone, __modified_by text)
    rows 1
    language plpgsql
as
...
create or replace function auth.lock_user_group(_modified_by text, _user_id bigint, _user_group_id integer, _tenant_id integer DEFAULT 1)
    returns TABLE(__user_group_id integer, __is_active boolean, __is_assignable boolean, __modified timestamp with time zone, __modified_by text)
    rows 1
    language plpgsql
as
...
create or replace function auth.unlock_user_group(_modified_by text, _user_id bigint, _user_group_id integer, _tenant_id integer DEFAULT 1)
    returns TABLE(__user_group_id integer, __is_active boolean, __is_assignable boolean, __modified timestamp with time zone, __modified_by text)
    rows 1
    language plpgsql
as
...
create or replace function auth.set_user_group_as_hybrid(_modified_by text, _user_id bigint, _user_group_id integer, _tenant_id integer DEFAULT 1) returns void
    language plpgsql
as
$$
begin
...
create or replace function auth.get_tenants(_user_id bigint)
    returns TABLE(__created timestamp with time zone, __created_by text, __modified timestamp with time zone, __modified_by text, __tenant_id integer, __uuid text, __title text, __code text, __is_removable boolean, __is_assignable boolean)
    language plpgsql
as
$$
begin
...
create or replace function unsecure.set_permission_as_assignable(_modified_by text, _user_id bigint, _permission_id integer DEFAULT NULL::integer, _permission_full_code text DEFAULT NULL::text, _is_assignable boolean DEFAULT true) returns SETOF auth.permission_assignment
    language plpgsql
as
$$
declare
...
create or replace function auth.set_permission_as_assignable(_modified_by text, _user_id bigint, _permission_id integer DEFAULT NULL::integer, _permission_full_code text DEFAULT NULL::text, _is_assignable boolean DEFAULT true) returns SETOF auth.permission_assignment
    language plpgsql
as
$$
begin
...
create or replace function unsecure.update_perm_set(_modified_by text, _user_id bigint, _perm_set_id integer, _title text, _is_assignable boolean DEFAULT true, _tenant_id integer DEFAULT 1) returns SETOF auth.perm_set
    rows 1
    language plpgsql
as
$$
...
create or replace function auth.update_perm_set(_modified_by text, _user_id bigint, _perm_set_id integer, _title text, _is_assignable boolean DEFAULT true, _tenant_id integer DEFAULT 1) returns SETOF auth.perm_set
    rows 1
    language plpgsql
as
$$
...
create or replace function auth.enable_user(_modified_by text, _user_id bigint, _target_user_id bigint)
...
create or replace function auth.disable_user(_modified_by text, _user_id bigint, _target_user_id bigint)
    returns TABLE(__user_id bigint, __is_active boolean, __is_locked boolean)
    rows 1
    language plpgsql
as
...
create or replace function auth.unlock_user(_modified_by text, _user_id bigint, _target_user_id bigint)
    returns TABLE(__user_id bigint, __is_active boolean, __is_locked boolean)
    rows 1
    language plpgsql
as
...
create or replace function auth.lock_user(_modified_by text, _user_id bigint, _target_user_id bigint)
    returns TABLE(__user_id bigint, __is_active boolean, __is_locked boolean)
    rows 1
    language plpgsql
as
...
create or replace function auth.enable_user_identity(_modified_by text, _user_id bigint, _target_user_id bigint, _provider_code text)
...
create or replace function auth.disable_user_identity(_modified_by text, _user_id bigint, _target_user_id bigint, _provider_code text)
    returns TABLE(__user_identity_id bigint, __is_active boolean)
    rows 1
    language plpgsql
as
...
create or replace function unsecure.update_user_password(_modified_by text, _user_id bigint, _target_user_id bigint, _password_hash text DEFAULT NULL::text, _password_salt text DEFAULT NULL::text)
...
create or replace function auth.update_user_password(_modified_by text, _user_id bigint, _target_user_id bigint, _password_hash text, _ip_address text, _user_agent text, _origin text, _password_salt text DEFAULT NULL::text)
    returns TABLE(__user_id bigint, __provider_code text, __provider_uid text)
    rows 1
    language plpgsql
as
...
create or replace function auth.update_user_data(_modified_by text, _user_id bigint, _target_user_id bigint, _provider text, _user_data jsonb)
...
create or replace function auth.search_api_keys(_user_id bigint, _search_text text, _page integer DEFAULT 1, _page_size integer DEFAULT 10, _tenant_id integer DEFAULT 1)
    returns TABLE(__created_by text, __created timestamp with time zone, __modified_by text, __modified timestamp with time zone, __api_key_id integer, __tenant_id integer, __title text, __description text, __api_key text, __expire_at timestamp with time zone, __notification_email text, __total_items bigint)
    stable
    rows 100
    language plpgsql
as
...
create or replace function auth.set_token_as_used(_modified_by text, _user_id bigint, _token_uid text, _token text, _token_type_code text, _ip_address text, _user_agent text, _origin text)
...
create or replace function auth.set_token_as_used_by_token(_modified_by text, _user_id bigint, _token text, _token_type text, _ip_address text, _user_agent text, _origin text)
    returns TABLE(__token_id bigint, __token_uid text, __token_state_code text, __used_at timestamp with time zone, __user_id bigint, __user_oid text, __token_data jsonb)
    language plpgsql
as
$$
...
create or replace function public.validate_token(_modified_by text, _user_id bigint, _target_user_id bigint, _token_uid text, _token text, _token_type text, _ip_address text, _user_agent text, _origin text, _set_as_used boolean DEFAULT false)
...
create or replace function auth.set_token_as_failed(_modified_by text, _user_id bigint, _token_uid text, _token text, _token_type_code text, _ip_address text, _user_agent text, _origin text)
    returns TABLE(__token_id bigint, __token_uid text, __token_state_code text, __used_at timestamp with time zone, __user_id bigint, __user_oid text, __token_data jsonb)
    language plpgsql
as
$$
...
create or replace function auth.set_token_as_failed_by_token(_modified_by text, _user_id bigint, _token text, _token_type text, _ip_address text, _user_agent text, _origin text)
    returns TABLE(__token_id bigint, __token_uid text, __token_state_code text, __used_at timestamp with time zone, __user_id bigint, __user_oid text, __token_data jsonb)
    language plpgsql
as
$$
...
create or replace function auth.set_user_group_as_external(_modified_by text, _user_id bigint, _user_group_id integer, _tenant_id integer DEFAULT 1) returns void
    language plpgsql
as
$$
declare
...
create or replace function auth.validate_token(_modified_by text, _user_id bigint, _target_user_id bigint, _token_uid text, _token text, _token_type_code text, _ip_address text, _user_agent text, _origin text, _set_as_used boolean DEFAULT false)
    returns TABLE(___token_id bigint, ___token_uid text, ___token_state_code text, ___used_at timestamp with time zone, ___user_id bigint, ___user_oid text, ___token_data jsonb)
    language plpgsql
as
$$
...
create or replace function public.search_journal_msgs(_user_id bigint, _search_text text, _from timestamp with time zone DEFAULT NULL::timestamp with time zone, _to timestamp with time zone DEFAULT NULL::timestamp with time zone, _target_user_id integer DEFAULT NULL::integer, _event_id integer DEFAULT NULL::integer, _data_group text DEFAULT NULL::text, _data_object_id bigint DEFAULT NULL::bigint, _data_object_code text DEFAULT NULL::text, _payload_criteria jsonb DEFAULT NULL::jsonb, _page integer DEFAULT 1, _page_size integer DEFAULT 10, _tenant_id integer DEFAULT 1)
    returns TABLE(__created timestamp with time zone, __created_by text, __journal_id bigint, __event_id integer, __data_group text, __data_object_id bigint, __data_object_code text, __user_id bigint, __msg text, __total_items bigint)
    stable
    language plpgsql
as
$$
...
create or replace function auth.update_user_preferences(_updated_by text, _user_id bigint, _target_user_id bigint, _update_data text)
    returns TABLE(__modified timestamp with time zone, __modified_by character varying)
    rows 1
    language plpgsql
as
$$
...
create or replace function auth.create_user_tenant_preferences(_created_by text, _user_id bigint, _target_user_id bigint, _update_data text, _tenant_id integer DEFAULT 1)
    returns TABLE(__created timestamp with time zone, __created_by character varying)
    rows 1
    language plpgsql
as
$$
