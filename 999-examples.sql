-- Set correlation ID for tracing (change to a real value for testing, e.g. \set correlation_id '''my-trace-id''')
\set correlation_id 'initial'

set search_path = public, const, ext, stage, helpers, internal, unsecure, auth, triggers;

select *
from auth.ensure_user_from_provider(_created_by := 'system', _user_id := 1, _correlation_id := :correlation_id, _provider_code := 'aad',
...
select *
from auth.register_user('registrator', 1, :correlation_id, 'lucie.novakova1@keenmate.com', '123456', _display_name := 'Lucie Novakova',
												_user_data := '{
													"firstName": "Lucie",
													"lastname": "Novakova"
												}');
...
select *
from auth.get_provider_users('system', 1, :correlation_id, 'aad');

select *
from auth.get_provider_users('system', 1, :correlation_id, 'email');

select *
from unsecure.create_user_group_member_as_system('ondrej.valenta@keenmate.com', 'Tenant admins', _tenant_id := 1);

select *
from auth.create_tenant('ondrej.valenta', 1000, :correlation_id, 'Albert Moravec', _tenant_owner_id := 1001);

select *
from auth.create_owner('ondrej.valenta', 1, :correlation_id, 1002, 1, 2);

select *
from auth.create_user_group('filip.jakab', 1002, :correlation_id, 'Our customers', _tenant_id := 2);

-- albert.moravec, owner of the tenant, creates a new user group member (jan.rada)
select *
from auth.create_user_group_member('albert.moravec', 1001, :correlation_id, 5, 1004, _tenant_id := 2);

-- create tenant: Jan Rada, Account ondrej.valenta is not the owner, so the account has not permissions
select *
from auth.create_tenant('ondrej.valenta', 1000, :correlation_id, 'Jan Rada');

-- assign tenant owner to user jan.rada as system account
select *
from auth.create_owner('ondrej.valenta', 1000, :correlation_id, 1003, _tenant_id := 3);

-- jan.rada adds ondrej.valenta@keenmate.com as member of Tenant Owner of tenant: Jan Rada
select *
from auth.create_owner('jan.rada', 1003, :correlation_id, 1000, _tenant_id := 3);

-- create an external group with mapping to aad_rada in aad auth provider in tenant: Jan Rada
select *
from auth.create_external_user_group('system', 1000, :correlation_id, 'External group 1', 'aad', _mapped_object_id := 'aad_rada',
																	 _tenant_id := 3);
...
select *
from auth.delete_perm_set_permissions('ondrej.valenta', 1::bigint, :correlation_id, 6,
...
select *
from auth.ensure_groups_and_permissions('authenticator', 1, :correlation_id, 1003, 'aad', array ['aad_rada'], _tenant_id := 3);
...
select *
from auth.has_permission(1003, :correlation_id, 'areas.public', _tenant_id := 3);
-- user permission check does not change for user: Jan Rada for 15 seconds and then on next check it is reevaluated

select *
from auth.get_tenant_users('system', 1, :correlation_id, 3);

select *
from auth.get_tenant_groups('system', 1, :correlation_id, 1);

select *
from auth.user_group_members;

select *
from auth.disable_user('kerberos', 1, :correlation_id, 1004);

select *
from auth.enable_user('kerberos', 1, :correlation_id, 1004);

select *
from auth.lock_user('kerberos', 1, :correlation_id, 1004);

select *
from auth.unlock_user('kerberos', 1, :correlation_id, 1004);

select *
from auth.disable_user_identity('kerberos', 1, :correlation_id, 1004, 'email');

select *
from auth.enable_user_identity('kerberos', 1, :correlation_id, 1004, 'email');


select *
from auth.get_user_by_email_for_authentication(1, :correlation_id, 'lucie.novakova1@keenmate.com');


select *
from auth.create_user_event('authenticator', 1, :correlation_id, 'email_verification', 1004,
														'123.123.232.12', 'the best user agent there is',
														'domain.com');

select *
from auth.create_token('authenticator', 1, :correlation_id, 1004, null, null, 'email_verification', 'email', '111jjjj2222jjjj333');

--4FDC32F629CE
select *
from auth.validate_token('authenticator', 1, :correlation_id, null, null, '111jjjj2222jjjj333', 'email_verification', '123.2.34.5',
												 'my agent', 'keenmate.com',
												 true);

select *
from auth.get_user_group_members('ondrej', 1, :correlation_id, 8, 3);

select *
from auth.get_tenant_members('ondrej', 1, :correlation_id, 3);

select *
from auth.get_tenant_groups('ondrej', 1, :correlation_id, 3);


select *
from auth.create_user_group_member('system', 1, :correlation_id, 1, 1000);

select *
from auth.has_permission(1000, :correlation_id, 'users.create_service_user', 1, false);
select *
from auth.has_permission(1001, :correlation_id, 'users.create_service_user', 1, false);

select *
from auth.permission;

select *
from auth.create_service_user_info('ondrej.valenta@keenmate.com', 1000, :correlation_id, 'export_service_2', 'Export service 2');

select *
from auth.create_permission('system', 1, :correlation_id, 'Business', null);
select *
from auth.create_permission('system', 1, :correlation_id, 'Orders', 'business');
select *
from auth.create_permission('system', 1, :correlation_id, 'Create order', 'business.orders');

select *
from auth.create_perm_set('system', 1, :correlation_id
	, 'Export service'
	, _permissions := array ['business.orders.create_order']);

select *
from auth.create_api_key('system', 1, :correlation_id, 'Another service'
...
select *
from auth.get_user_permissions(1, :correlation_id, 1005);
...
select *
from auth.get_user_permissions(1, :correlation_id,);
...
select *
from add_journal_msg('ondrej', 1, :correlation_id
...
select *
from auth.delete_user_group_member('system', 2, :correlation_id, 1, 10, 2)
